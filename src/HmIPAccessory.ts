...
export class HmIPAccessory {
  ...
  public register() {
    if (this.isFromCache) {
      this.log.info(`Updating accessory: ${this.accessory.context.device.id} -> uuid ${this.accessory.UUID}`);
      this.api.updatePlatformAccessories([this.accessory]);
    } else {
      this.log.info(`Register accessory: ${this.accessory.context.device.id} -> uuid ${this.accessory.UUID}`);
